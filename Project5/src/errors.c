...
// the error codes.
const char *error_strings[] = {"SUCCESS",
                               "ERR_UNKNOWN_CMD",
                               "ERR_PIPE_READ_ERR",
                               "ERR_PIPE_WRITE_ERR",
                               "ERR_UNKNOWN_ACCOUNT",
...
void error_msg(int error, const char *msg) {
  err = error - 1;
  strncpy(errm, msg, ERROR_BUFFER_SIZE);
  errm[ERROR_BUFFER_SIZE - 1] = '\0';
}

const char *error_msg_str() { return errm; }

int error_type() { return err + 1; }

void error_print() { printf("ERROR %d %s\n", err + 1, error_strings[err]); }
